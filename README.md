# Daily Builds

A 30-minute-a-day practice in shipping. One small thing every day, varied across languages and domains. The point is the habit; the side effect is a record of how I think and what I'm curious about.

## Rotation

- **Mon** — Python utility
- **Tue** — Frontend mini-app
- **Wed** — CLI tool
- **Thu** — Data viz / notebook
- **Fri** — API / scraper
- **Sat** — Refactor or extend
- **Sun** — Learn one thing

## Log

| Date | Build | Stack | Notes |
|------|-------|-------|-------|
| 2026-05-28 | [Caregiving Economics](./2026-05-28-caregiving-economics) | Python, pandas, matplotlib | Unpaid care burden — bar + bubble scatter |
| 2026-05-27 | [Case Prep CLI](./2026-05-27-case-prep) | Python | Flashcard tool with weighted random repetition |
| 2026-05-26 | [Decision Matrix](./2026-05-26-decision-matrix) | HTML/JS | Live-updating weighted decision tool |
| 2026-05-25 | [Cost-Per-Hire Calc](./2026-05-25-cost-per-hire) | Python | CSV → recruiting metrics |
| 2026-05-24 | [Regex Demo](./2026-05-24-regex-demo) | Python | Three real-world regex patterns |
| 2026-05-23 | [Standup CLI v0.2](./2026-05-20-standup-cli) | Python | Refactor: tags + weekly markdown digest |
| 2026-05-22 | [Clinical Trials Snapshot](./2026-05-22-clinical-trials) | Python, requests | Live data from ClinicalTrials.gov API |
| 2026-05-21 | [Pharma R&D Productivity](./2026-05-21-pharma-rd-viz) | Python, pandas, matplotlib | Decade of spend vs. approvals |
| 2026-05-20 | [Standup CLI](./2026-05-20-standup-cli) | Python | Daily standup logger, JSON persistence |
| 2026-05-19 | [Pitch Timer](./2026-05-19-pitch-timer) | HTML/JS | Single-file timer for pitch practice |